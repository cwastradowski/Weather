...
type
  ...
  private
    procedure PopulateGridTextStyle(iFirstLine: Integer; iMaxStart: Integer;
       iMaxLength: Integer; iMinStart: Integer; iMinLength: Integer;
       iRainStart: Integer; iRainLength: Integer; sl: TStringList; iDaysInMonth: Integer);
    procedure PopulateGridCSVStyle(iFirstLine: Integer; iMaxStart: Integer;
       iMaxLength: Integer; iMinStart: Integer; iMinLength: Integer;
       iRainStart: Integer; iRainLength: Integer; sl: TStringList; iDaysInMonth: Integer);
...
type TImportElements = Record
  ...
  Comments : TImportElementsItem; //used for file type
End;
...
begin
  ...
  if ImportElements.Comments.FileType = 'txt' then
    PopulateGridTextStyle(iFirstLine, iMaxStart, iMaxLength, iMinStart, iMinLength,
      iRainStart, iRainLength, sl, iDaysInMonth);
  if ImportElements.Comments.FileType = 'csv' then
    PopulateGridCSVStyle(iFirstLine, iMaxStart, iMaxLength, iMinStart, iMinLength,
      iRainStart, iRainLength, sl, iDaysInMonth);
...
begin
  ...
  for i := 0 to length(frmMain.aCDS) - 1 do
  begin
    frmMain.aCDS[i].Refresh;
    frmMain.aCDS[i].Last;
  end;
end;
...
begin
  ...
  // 9 File Type
  simpleDSGetImportElements.Locate('idElement',9,[loCaseInsensitive]);
  ImportElements.Comments.FileType := Trim(simpleDSGetImportElements.FieldByName('sComments').AsString);
  //Clean Up
  simpleDSGetImportElements.Active := False;
  simpleDSGetImportElements.Free;
end;

procedure TfrmImport.PopulateGridTextStyle(iFirstLine: Integer; iMaxStart: Integer;
          iMaxLength: Integer; iMinStart: Integer; iMinLength: Integer;
          iRainStart: Integer; iRainLength: Integer; sl: TStringList; iDaysInMonth: Integer);
var
  i: Integer;
...
procedure TfrmImport.PopulateGridCSVStyle(iFirstLine: Integer; iMaxStart: Integer;
          iMaxLength: Integer; iMinStart: Integer; iMinLength: Integer;
          iRainStart: Integer; iRainLength: Integer; sl: TStringList; iDaysInMonth: Integer);
var
  i,j: Integer;
  sFileLine, sFileLineTrim,sFileLineSave : string;
...
begin
  ...
  begin
    ...
    // Maximum
        sFileLine := sl.Strings[iFirstLine + i - 2];
        sFileLineSave := copy(sFileLine,Pos(',',sFileLine) + 1,80);

        for j := 1 to iMaxStart do   //iMaxStart is the number of rows to move
        begin
          sFileLineTrim := copy(sFileLineSave,1,Pos(',',sFileLineSave)-1);
          sFileLineSave := copy(sFileLineSave,Pos(',',sFileLineSave)+ 1,80);
        end;
        sgridData.Cells[1, i] := sFileLineTrim;

    // Minimum
        sFileLine := sFileLineSave;
        for j := 1 to iMinStart  do  //iMinStart is the number of rows to move
        begin
          sFileLineTrim := copy(sFileLineSave,1,Pos(',',sFileLineSave)-1);
          sFileLineSave := copy(sFileLineSave,Pos(',',sFileLineSave)+ 1,80);
        end;
        sgridData.Cells[2, i] := sFileLineTrim;

    // Rain
        for j := 1 to iRainStart do   //iRainStart is the number of columns to move
        begin
          //Trim the line to get to the rain fiels
          sFileLineSave := copy(sFileLineSave,Pos(',',sFileLineSave)+ 1,80);
        end;

        if Pos(',',sFileLineSave) > 0 then //remove the text after the rain field
         begin
            sFileLineSave := copy(sFileLineSave,1,Pos(',',sFileLineSave)-1);
         end;

        if UpperCase(Trim(Copy(sFileLineSave, 1, 4))) = 'T' then
          sgridData.Cells[3, i] := '0.00'
        else
          sgridData.Cells[3, i] := sFileLineSave;
        end;
  sl.Free;
end;
