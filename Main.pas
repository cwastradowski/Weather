...
type
  TfrmMain = class(TForm)
    ...
    procedure cbCompareToDateClick(Sender: TObject);
  ...
  public
    ...
    aTabSheetStatsPage1, aTabSheetStatsPage2, aTabSheetStatsPage3,
      aTabSheetStatsPage4,aTabSheetStatsPage5: array of TTabSheet;
    ...
    aStringGridStatsPage1, aStringGridStatsPage2, aStringGridStatsPage3,
      aStringGridStatsPage4,aStringGridStatsPage5: array of TStringGrid;
...
var
  ...
  wQuerySavedEndYear, wQuerySavedEndMonth, wQuerySavedEndDay: Word;
  dtQuerySavedStartDate : TDateTime;
  dtQuerySavedEndDate : TDateTime;
  iChartDays: Integer;
...
uses unitStats, unitWebBrowser, unitSetup, unitImport,mpcAbout;
...
begin
  ...
  // Connection String
  sAboutServerName := mpcGetComputerName;
  sAboutUserName := mpcGetUserName;
  StatusBar1.Panels[0].Text :=  sAboutServerName + '\' + sAboutUserName;
  ...
  dtQuerySavedEndDate := calQueryEndDate.Date;
  dtQuerySavedStartDate := calQueryEndDate.Date;
...
begin
  ...
  begin
    ...
    aURLBtn[i].ShowHint := True;
    aURLBtn[i].Hint := Trim(aStationInfo[i].sNWSUrl);
...
begin
  ...
  SetLength(aTabSheetStatsPage5, nStationCount);
  ...
  SetLength(aStringGridStatsPage5, nStationCount);
  ...
  begin
    ...
    aTabSheetStatsPage5[i] := TTabSheet.Create(self);
    aTabSheetStatsPage5[i].PageControl := aPGCTRStatsPages[i];
    aTabSheetStatsPage5[i].Caption := 'Page 5';
    ...
    aStringGridStatsPage5[i] := TStringGrid.Create(self);
    aStringGridStatsPage5[i].Parent := aTabSheetStatsPage5[i];
    aStringGridStatsPage5[i].Top := 0;
    aStringGridStatsPage5[i].Left := 0;
    aStringGridStatsPage5[i].Width := 500;
    aStringGridStatsPage5[i].Height := 200;
    aStringGridStatsPage5[i].DefaultColWidth := 100;
    aStringGridStatsPage5[i].ColWidths[0] := 70;
    aStringGridStatsPage5[i].DefaultRowHeight := 17;
    aStringGridStatsPage5[i].ScrollBars := ssBoth;
    aStringGridStatsPage5[i].ColCount := 4;
    aStringGridStatsPage5[i].RowCount := 4;
    aStringGridStatsPage5[i].FixedRows := 1;
    aStringGridStatsPage5[i].FixedCols := 1;
    aStringGridStatsPage5[i].Tag := i;
...
procedure TfrmMain.UpdateQueryCaption;
begin
  lblQueryRange.Caption := 'Stats for dates from ' + DateToStr(calQueryStartDate.Date) + ' to ' + DateToStr(calQueryEndDate.Date) + '.';
end;
{$ENDREGION}
...
var
  sURL : string;
begin
  sURL := aStationInfo[TButton(Sender).Tag].sNWSUrl;
...
begin
  DecodeDate(calQueryEndDate.Date, wQuerySavedEndYear, wQuerySavedEndMonth, wQuerySavedEndDay);
...
procedure TfrmMain.cbCompareToDateClick(Sender: TObject);
begin
  DecodeDate(calQueryStartDate.Date, wQueryBeginYear, wQueryBeginMonth,
             wQueryBeginDay);
if not cbCompareToDate.Checked  then
  begin
    calQueryEndDate.Date := StrToDate('12/31/' + IntToStr(wQuerySavedEndYear));
  end
  else
    calQueryEndDate.Date := dtQuerySavedEndDate;
  begin
...
procedure TfrmMain.actAboutBoxExecute(Sender: TObject);
var
  bExists: boolean;
  i: Integer;
begin
